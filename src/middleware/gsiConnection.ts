...
export async function handleMorphlingEvents(events: MorphlingEvent[], clientId: number): Promise<void> {
    if(events.length > 0 ) {
        ...
        if(gameStateChange && activity && gameStateChange.value === GameState.preGame && (activity.value === 'playing' || (user?.useAutomaticVoting))) {
        ...
        if(winner && winner.value.winnerTeam !== 'none') {
            ...
            await resolveBet(channel, clientId, winner.value.winnerTeam === 'radiant' ? (user?.teamAName.toLowerCase() || 'a') : (user?.teamBName.toLowerCase() || 'b'));
        }
    }
}