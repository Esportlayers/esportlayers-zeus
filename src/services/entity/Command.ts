...
import { clearUserCommandsChache } from "../chat/message";
...
export async function getUserCommands(userId: number): Promise<Command[]> {
  ...
  const [commandRows] = await conn.execute<Array<Command & RowDataPacket>>(
    `
        ...
        FROM bot_commands WHERE user_id = ?`,
    [userId]
  );
  await conn.end();
...
export async function patchCommand(
  ...
  for (const [key, value] of Object.entries(data)) {
    if (transformMap[key]) {
      await conn.execute(
        `UPDATE bot_commands SET ${transformMap[key]} = ? WHERE id=? AND user_id=?`,
        [value, commandId, userId]
      );
    }
  }
...
export async function deleteCommand(
  ...
  const conn = await getConn();
  await conn.execute("DELETE FROM bot_commands WHERE id=? AND user_id=?", [
    commandId,
    userId,
  ]);
  await conn.end();
...
export async function createBetCommands(userId: number): Promise<void> {
  ...
  const hasBetCommands =
    userCommands.filter(
      ({ type }) => type === "betting_streamer" || type === "betting_user"
    ).length > 0;
  ...
  if (!hasBetCommands) {
    await createUserCommand(
      ...
      true,
      "!startvote",
      "Das Voting sind offen, es kann nun mit “{BET_COMMAND} {TEAM_A}” oder “{BET_COMMAND} {TEAM_B}” abgestimmt werden",
      "betting_streamer",
      false,
      false,
      "startbet",
      new Set(["streamerAccess"])
    );
    await createUserCommand(
      userId,
      true,
      "!winner",
      "Der Gewinner des Votings wurde auf Team {WINNER} festgelegt.",
      "betting_streamer",
      false,
      false,
      "betwinner",
    ...
    await createUserCommand(
      ...
      true,
      "!toplist",
      "Die aktuelle Toplist ist: {TOPLIST_STATS}",
      "betting_user"
    );
    await createUserCommand(
      userId,
      true,
      "!votestats",
      "{USER}, du hast aktuell {USER_BETS_CORRECT} von {USER_BETS_TOTAL} Votings korrekt.",
      "betting_user"
    );
  }
}
