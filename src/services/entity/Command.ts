...
export async function createBetCommands(userId: number): Promise<void> {
    ...
    if(!hasBetCommands) {
        await createUserCommand(userId, true, '!startbet', 'Die Wetten sind offen, es kann nun mit “{BET_COMMAND} {TEAM_A}” oder “{BET_COMMAND} {TEAM_B}” abgestimmt werden', 'betting_streamer', false, false, 'startbet', new Set(['streamerAccess']));
        await createUserCommand(userId, true, '!winner', 'Der Gewinner der Wette wurde auf Team {WINNER} festgelegt.', 'betting_streamer', false, false, 'betwinner', new Set(['streamerAccess']));
