...
export async function getUserTimer(userId: number): Promise<Command[]> {
    const conn = await getConn();
    const [commandRows] = await conn.execute<Array<Command & RowDataPacket>>('SELECT id, period, message, active FROM bot_timer WHERE user_id = ?', [userId]);
    await conn.end();
