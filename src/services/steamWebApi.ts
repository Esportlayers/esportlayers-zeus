...
async function query(url: string, params: object = {}): Promise<any> {
  ...
  const qry = Object.entries(allParams)
    .map(([key, val]) => `${key}=${val}`)
    .join("&");
...
export async function fetchGame(
  matchId: number
): Promise<LiveLeagueGame | null> {
  const response = await query("/GetLiveLeagueGames/v1");

  if (response.ok) {
    const raw = await response.text();
    const data = (JSONbig.parse(raw) as unknown) as {
      result: { games: LiveLeagueGame[] };
    };
...
export async function fetchMatchDetails(
  ...
  if (game) {
    if (game.radiant_team && game.dire_team) {
      const radiantLogoUrl = await fetchTeamLogo(
        "" + game.radiant_team.team_logo
      );
      ...
      return {
        radiant: {
          ...
          logo: radiantLogoUrl,
          wins: game.radiant_series_wins,
        },
        dire: {
          name: game.dire_team.team_name,
          logo: direLogoUrl,
          wins: game.dire_series_wins,
        },
